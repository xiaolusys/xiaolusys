...
class JimayAgent(models.Model):
    ...
    is_graduated = models.BooleanField(default=False, db_index=True, verbose_name='是否结业')
    time_graduated_st = models.DateTimeField(default=None, blank=True, null=True, verbose_name='上课时间')
    time_graduated_en = models.DateTimeField(default=None, blank=True, null=True, verbose_name='结业时间')
    ...
    def gen_certification_filename(self):
        ...
        return 'cert-{mobile}-{sha1}'.format(mobile=self.mobile, sha1=sha1_str)

    def gen_awardgraduated_filename(self):
        agent_key_str = '%s-%s-%s-%s' % (self.name, self.idcard_no, self.weixin, self.level)
        sha1_str = hashlib.sha1(agent_key_str).hexdigest()
        return 'award-{mobile}-{sha1}'.format(mobile=self.mobile, sha1=sha1_str)

    @classmethod
    def gen_pay_qrcode_cache_key(cls, wx_pubid, pay_key):
        return 'jimay-payqrcode-{}-{}'.format(wx_pubid, pay_key)

    def gen_graduated_cert_cache_key(self):
        return 'jimay-award-{}-{}'.format(self.id, self.time_graduated_en)
