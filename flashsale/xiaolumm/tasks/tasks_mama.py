...
def task_update_second_level_ordercarry(referal_relationship, order_carry):
    ...
    records = OrderCarry.objects.filter(order_id=order_carry.order_id, carry_type__in=
        [OrderCarry.REFERAL_ORDER, OrderCarry.ADVANCED_MAMA_REFERAL_ORDER])
    if records.exists():
                ...
                if relationship:
                    ...
                    if upper_mama:
                        if upper_mama.is_elite_mama and upper_mama.elite_score < \
                                constants.ELITEMM_DESC_INFO[constants.ELITEMM_VP].get('min_score'):
                            diff = get_level_differential(model_product, upper_mama.elite_level, mm_linkid_mama.elite_level)
                            ...
                            can_exchg_payment = get_level_price_from_boutique_modelproduct(model_product, low_mama.elite_level)
                            # 为了便于存储，单位用分
