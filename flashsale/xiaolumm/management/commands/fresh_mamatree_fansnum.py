...
def recursive_appendchilds(node, node_maps, gradient):
    ...
    copy_node['direct_fans_num'] = len(copy_node['childs'])
    copy_node['indirect_fans_num'] = sum([c['direct_fans_num'] for c in copy_node['childs']])
    return copy_node


def get_referalrelationship_maps():
    referals = ReferalRelationship.objects.filter(
        status=ReferalRelationship.VALID).order_by('referal_from_mama_id')
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        print 'fetch mama top ids:', datetime.datetime.now(), mama_ids
