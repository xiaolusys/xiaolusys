...
from pms.supplier import constants, forms
...
def chargeSupplier(request, pk):
    ...
    if charged:
        result = {'success': True,
                  'brand_links':
                      '/pms/supplier/product/?status=wait&sale_supplier=%s'
                      % pk}
...
class SaleProductList(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return HttpResponseRedirect(
            "/pms/supplier/product/?status=%s&sale_supplier=%s" %
...
class SaleProductAdd(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        log_action(request.user.id, sproduct, ADDITION, u'创建品牌商品')
        return HttpResponseRedirect(
            "/pms/supplier/line_product/?status=%s&sale_supplier=%s" %
            (sproduct.status, supplier_id))
