...
class AddItemView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        while True:
            ...
            test_pros = Product.objects.filter(
                outer_id__startswith=inner_outer_id)
            if test_pros.count() == 0 or count > 998:
                break
            count += 1
        if len(inner_outer_id) > 12:
            return Response({"result": "编码生成错误"})
        model_pro = ModelProduct(name=product_name,
        ...
        all_chi_ma = [] if content.get("all_chima",
                                       "") == "" else chi_ma_str.split(",")
        chi_ma_result = {}
        for sku in all_sku:
        ...
        for color in all_colors:
            ...
            one_product = Product(name=product_name + "/" + color,
                                  outer_id=inner_outer_id + str(pro_count),
                                  model_id=model_pro.id,
                                  sale_charger=user.username,
                                  category=category_item,
                                  remain_num=total_remain_num,
                                  cost=cost,
                                  agent_price=agentprice,
                                  std_sale_price=price,
                                  ware_by=int(ware_by),
                                  sale_time=shelf_time,
                                  pic_path=header_img,
            ...
            one_product_detail = Productdetail(
                ...
                wash_instructions=wash_instroduce,
                note=note)
            one_product_detail.save()
            chima_model = ProductSkuContrast(product=one_product,
            ...
            for sku in all_sku:
                ...
                price = content.get(color + "_" + sku + "_pricestd", "")
                agentprice = content.get(color + "_" + sku + "_agentprice", "")
                one_sku = ProductSku(outer_id=outer_id,
                                     product=one_product,
