...
class LogisticsHandler(BaseHandler):
    ...
    def getGZLogisticCompany(self, state, city, district, shipping_type, receiver_address):
    ...
    def getSHLogisticCompany(self, state, city, district, shipping_type, receiver_address):
        if not state or not city or not district:
            raise Exception(u"地址不全(请精确到省市区（县）)")
    ...
    def process(self, package_order, *args, **kwargs):

        logger.debug('DEBUG LOGISTIC:%s' % package_order)

        try:
            # 如果订单属于广州仓，则默认发韵达
            if package_order.ware_by == pcfg.WARE_GZ:
                package_order.logistics_company = self.getGZLogisticCompany(
                    package_order, package_order.state, package_order.city, )
            else:
                package_order.logistics_company = self.getSHLogisticCompany(package_order)
            update_model_fields(package_order, update_fields=['logistics_company', 'ware_by'])
        except Exception, exc:
            package_order.sys_memo += u'[物流：%s]' % exc.message
            update_model_fields(package_order, update_fields=['sys_memo'])
            package_order.append_reason_code(pcfg.DISTINCT_RULE_CODE)
