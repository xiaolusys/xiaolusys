...
def get_mergeorder_by_refundproduct(refund_product):
    ...
    package_sku_item = PackageSkuItem.objects.filter(oid=trade_id, outer_id=refund_product.outer_id,
...
def get_refund_by_refundproduct(refund_product):
    package_sku_item = get_mergeorder_by_refundproduct(refund_product)
...
def update_Unrelate_Prods_Product(pro, req):
    ...
    if refund:  # 如果系统已经创建该退款单
        ...
        package_sku_item = get_mergeorder_by_refundproduct(pro)
        if not package_sku_item:
        ...
        try:
            ...
            refund = Refund()
            refund.tid = package_sku_item.sale_order_id  # 交易ID
            ...
            refund.buyer_nick = PackageOrder.objects.get(pid=package_sku_item.package_order_pid).buyer_nick  # 买家昵称
            ...
            refund.phone = PackageOrder.objects.get(pid=package_sku_item.package_order_pid).receiver_phone  # 电话
