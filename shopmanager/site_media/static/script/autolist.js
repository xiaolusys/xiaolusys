...
autolist.Dialog.prototype.init = function () {
    this.dialog.setContent(
	    ...
	    '<div class="controls">' +
            '<input id="id-list-time" type="text" value="22:00">' +
            '</div>' +
            '</div>');
    this.dialog.setTitle('修改上架时间');
...
autolist.ClickCell = function (cellDom, parent) {
    this.parent = parent;
    this.cell = cellDom;
    this.dialogButton = this.cell.childNodes[0];
...
autolist.ClickCell.prototype.handleEvent = function (e) {
	...
	this.cell.childNodes[0].style.display = "block";
	this.cell.childNodes[1].style.display = "none";
	return;
    }
    if (e.type == goog.events.EventType.MOUSEOUT) {
	this.cell.childNodes[0].style.display = "none";
	this.cell.childNodes[1].style.display = "block";
	return;
    }
