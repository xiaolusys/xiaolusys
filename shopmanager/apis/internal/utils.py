...
def get_multi_model_by_ids(filters, model_class):
    ...
    cache_values  = [m for m in cache_results.values() if m]
    match_keys = [getattr(m, filter_name) for m in cache_values]
    unmatch_keys = set(model_ids) - set(match_keys)

    for cache_key in unmatch_keys:
        try:
            cache_value = get_model_by_id({filter_name: cache_key}, model_class)
