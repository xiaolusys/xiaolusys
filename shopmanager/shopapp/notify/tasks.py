            ...
            #修改交易备注
                ...
                try:
                    ...
                    response    = apis.taobao_trade_fullinfo_get(tid=notify.tid,
                                                    fields='tid,buyer_message,seller_memo,seller_flag',tb_user_id=notify.user_id)
                    trade_dict  = response['trade_fullinfo_get_response']['trade']
    ...
    try:
        ...
        if notify.status == "ItemAdd":
            ...
            if outer_id:
                ...
                updateUserProductSkuTask(user_id=notify.user_id,outer_ids=[outer_id],force_update_num=False) #线上库存修改覆盖系统库存
