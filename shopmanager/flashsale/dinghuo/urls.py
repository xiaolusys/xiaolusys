...
,modify_return_goods_sku, delete_return_goods_sku, set_return_goods_sku_send, set_transactor, set_refund, export_return_goods, mark_unreturn
import views_wuliu
...
urlpatterns = [
    ...
    url(r'^changedetail/(?P<order_detail_id>\d+)/$', csrf_exempt(staff_member_required(ChangeDetailView.as_view())),
        name="changedetail"),
    url(r'^changedetail/(?P<order_detail_id>\d+)/export/$',
        csrf_exempt(staff_member_required(ChangeDetailExportView.as_view())), name="changedetail_export"),
    ...
    # 修改上架前库存
    url(r'^sale_warning/$', staff_member_required(views_sale_status.SaleStatusView.as_view()), name="sale_warning"),
    # 销售预警
    url(r'^set_remain_num/$', staff_member_required(views_product.SetRemainNumView.as_view()), name="set_remian"),
    # 设置预留数
    url(r'^product_category/$', staff_member_required(views.ProductCategoryAPIView.as_view()), name="product_category"),
    # 商品分类api
    url(r'^skuapi/$', staff_member_required(SkuAPIView.as_view()), name="product_category"),  # 商品分类api
    url(r'^stats/$', staff_member_required(views_line_show.InventoryDataLineShow.as_view()), name="line_show"),
    # 折线图显示数据
