...
class SaleFaqsViewSet(viewsets.ModelViewSet):
    ...
    def get_question_type(self):
        ...
        types = self.queryset.values('question_type', 'detail_type').annotate()
        question_types = {}
        # 先排序下类型元组
        ...
        for tu in types:
            if question_types.has_key(QUESTION_TYPE[tu['question_type']][1]):
                question_types[QUESTION_TYPE[tu['question_type']][1]].append(
                    DETAIL_TYPE[tu['detail_type']][1])
            else:
                question_types[QUESTION_TYPE[tu['question_type']][1]] = [DETAIL_TYPE[tu['detail_type']][1], ]
    ...
    def get_question(self, request):
        ...
        if question_type and detail_type:
            questions = self.queryset.filter(question_type=question_type, detail_type=detail_type)
