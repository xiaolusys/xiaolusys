...
def release_Coupon_11_11(sender, instance, created, **kwargs):
    ...
    if instance.buyer_id != 6:
        return

    now = datetime.datetime.now()
    logger.error(u'测试：%s--%s' % (start_time, end_time), exc_info=True)
    ...
    if order and order.item_id in ['22030', '14362', '2731']:  # 列表中填写 充值产品id
        logger.error(u'order：%s  item_id:%s' % (order, order.item_id), exc_info=True)
        return
    try:
        ...
        logger.error(u'没有优惠券创建:%s' % instance.status, exc_info=True)
