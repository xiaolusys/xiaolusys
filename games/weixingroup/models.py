...
class XiaoluAdministrator(BaseModel):
    ...
    @staticmethod
    def get_admins_by_active_no(active_no):
        return XiaoluAdministrator.objects.filter(
            username__startswith=active_no,
            status=1
        )
...
class GroupFans(BaseModel):
    ...
    group = models.ForeignKey(GroupMamaAdministrator, related_name=u'fans')
