...
from shopback.items.views.views_add import AddItemView, GetCategory, GetSupplier, GetSkuDetail, PreviewSkuDetail, \
    BatchSetTime, ProductScheduleView, ProductScheduleAPIView
from rest_framework import routers

from shopback.items.views.views import *
...
urlpatterns = [
    ...
    url(r'^split/$', TemplateView.as_view(template_name="items/split_product_template.html")),
    url(r'^product/(?P<id>[0-9]+)/$', ProductView.as_view()),
    url(r'^product/(?P<pid>[0-9]+)/(?P<sku_id>[0-9]+)/$', ProductSkuView.as_view()),

    url(r'^product/$', ProductListView.as_view({'get': 'list'})),
    url(r'^product/item/(?P<outer_id>[\w^_]+)/$', ProductItemView.as_view()),
    url(r'^product/modify/(?P<outer_id>[\w^_]+)/$', ProductModifyView.as_view()),
    url(r'^product/update/(?P<outer_id>[\w^_]+)/$', ProductUpdateView.as_view()),
    url(r'^product/sku/(?P<sku_id>[\w^_]+)/$', ProductSkuInstanceView.as_view()),
    url(r'^query/$', ProductSearchView.as_view()),
    url(r'^product/barcode/$', ProductBarCodeView.as_view()),
    url(r'^product/district/(?P<id>[0-9]+)/$', ProductDistrictView.as_view()),
    url(r'^podorsku/status/$', ProductOrSkuStatusMdView.as_view()),
    url(r'^product/warn/$', ProductWarnMgrView.as_view()),
    url(r'^product/assign/$', ProductNumAssignView.as_view()),
    url(r'^product/sale/$', StatProductSaleView.as_view()),
    url(r'^product/sale_async/$', StatProductSaleAsyncView.as_view()),
    url(r'^product/scan/$', ProductScanView.as_view()),
