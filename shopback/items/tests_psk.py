...
def create_product_category():
    ...
    product_category = {"cid": 23, "parent_cid": 0, "name": "古董/邮币/字画/收藏", "is_parent": 1, "status": "mormal",
                        "sort_order": 145, "grade": 0}
...
def create_sale_trade():
    ...
    sale_trade = {"id": 468462, "tid": "xd123123", "buyer_id": customer.id, "buyer_nick": "疾风剑豪", "payment": 27.9,
                  "receiver_name": "剑圣", "receiver_address": "德玛西亚",
...
def create_sale_order():
    ...
    sale_order = {"id": 248281, "oid": "xo123123", "item_id": 10270, "title": "衬衫", "price": 29.9, "sku_id": '248181',
                  "num": 1, "outer_id": '80205508201', "outer_sku_id": '4', "total_fee": 29.9,
                  "payment": 29.9, "sku_name": "XL", "status": 5, "sale_trade_id": 468462,
...
def create_sku_stock(product, product_sku):
    SkuStock.objects.filter(id=166779).delete()
    SkuStock.objects.filter(sku=product_sku).delete()
...
#         self.sku_stock = create_sku_stock(self.product, self.product_sku)
...
#         self.purchase_arragement = PurchaseArrangement.objects.filter(
#             package_sku_item_id=self.package_sku_item.id).first()
...
#         self.package_sku_item.set_status_booked_2_assigned(stat=False)
#         self.sku_stock = SkuStock.objects.get(id=self.sku_stock.id)
...
#         self.package_sku_item = PackageSkuItem.create(self.sale_order)
#         self.sku_stock = SkuStock.objects.get(id=self.sku_stock.id)
#         field_status = ['paid', 'prepare_book', 'booked', 'ready', 'assigned', 'merged', 'waitscan', 'waitpost', 'sent',
#                         'finish']
...
#         self.package_sku_item.save()
#         field = [self.package_sku_item.package_order_id, self.package_sku_item.package_order_pid,
#                  self.package_sku_item.assign_status]
