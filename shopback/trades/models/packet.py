...
class PackageOrder(models.Model):
    ...
    WAIT_OUTWARE_SEND_CALLBACK = PO_STATUS.WAIT_OUTWARE_SEND_CALLBACK
    ...
    def get_package_type_display(self):
        ...
        return res.get(self.package_type, '未知类型')
    ...
    def sale_orders(self):
        if not hasattr(self, '_sale_orders_'):
            from flashsale.pay.models import SaleOrder
            sale_order_ids = list(PackageSkuItem.objects.filter(
                package_order_id=self.id).values_list('sale_order_id', flat=True))
        ...
        try:
            for item in self.package_sku_items.all():
                if item.type == PSI_TYPE.NORMAL:
                    ...
                    trade = SaleTrade.objects.filter(tid=tid).first()
                    if trade and get_sale_trade_address_dict(trade) != get_package_address_dict(self):
                        return u'地址错误:' + str(self.pid) + '|' + str(tid)
                if item.type == PSI_TYPE.TIANMAO:
                    pass
                    # 这里要检查一下地址  @TODO@黄炎
                    # item.get_objects().
        except:
            return u'地址错误，未知异常:' + str(self.pid) + '|' + str(item and item.id or '')

    def refresh_stat(self):
        from shopback.trades.tasks import task_update_package_stat_num
        task_update_package_stat_num.delay(self.pstat_id)

    def cancel(self, update_package_sku_status=True):
        self.status = PO_STATUS.DELETE
        self.save()
        if update_package_sku_status:
...
class PackageSkuItem(BaseModel):
    ...
    def save(self, *args, **kwargs):
        # 如果sku订单被取消，则检查并取消包裹订单状态
        if self.assign_status == PackageSkuItem.CANCELED:
            if self.package_order and self.package_order.sys_status != PackageOrder.DELETE:
                po_psis = self.package_order.package_sku_items.exclude(assign_status=PackageSkuItem.CANCELED)
                if not po_psis.exists():
                    self.package_order.sys_status = PackageOrder.DELETE
                    self.package_order.save(update_fields=['sys_status'])

        return super(PackageSkuItem, self).save(*args, **kwargs)
    ...
    def sale_order(self):
        if not hasattr(self, '_sale_order_'):
            from flashsale.pay.models import SaleOrder
            self._sale_order_ = SaleOrder.objects.filter(id=self.sale_order_id).first()
