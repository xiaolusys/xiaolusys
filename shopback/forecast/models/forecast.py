...
from shopback.dinghuo.models import OrderList, OrderDetail, InBound, OrderDetailInBoundDetail
...
class ForecastInbound(BaseModel):
    ...
    forecast_no = models.CharField(max_length=32, default=default_forecast_inbound_no,
                                   unique=True, verbose_name=u'预测单编号')
    ...
    def merge(orderlist_ids):
        ...
        for forcast in forcasts:
            for order_list in forcast.relate_order_set.all():
                try:
...
#     from shopback.dinghuo.tasks import task_update_order_group_key
