...
def json(obj, fields=[]):
    if isinstance(obj, QuerySet):
        if fields == []:
            return [dict(zip([f.name for f in o._meta.fields], [unicode(getattr(o, f.name)) for f in o._meta.fields]))
                    for o in obj]
        return [dict(zip([f.name for f in fields], [unicode(getattr(o, f.name)) for f in fields])) for o in obj]
    else:
        if fields == []:
            return dict(
                zip([f.name for f in obj._meta.fields], [unicode(getattr(obj, f.name)) for f in obj._meta.fields]))
